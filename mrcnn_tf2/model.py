...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    """
    def sigmoid(x, thresh=0.3, mult=5.0):
        return 1/(1+np.exp(mult*(thresh-x)))

    fgprobs = tf.math.sigmoid(-5.0*(config.DETECTION_FG_THRESHOLD-probs[:,1]))
    bgprobs = 1.0 - probs[:,1]
    probs = tf.stack([bgprobs, fgprobs], axis=1)
